...
- Use `tests/AgendaPro.Tests` for tests that need Infrastructure concerns such as EF Core InMemory or repository-level behavior. This project currently targets `net10.0` and references `AgendaPro.Domain` plus `AgendaPro.Infrastucture`.
...
- `tests/AgendaPro.Tests` targets `net10.0`.
- `Directory.Build.props` now defaults to `net10.0`, but always verify the effective framework in the specific test project's `.csproj` before editing.
- Keep package versions aligned with the .NET 10 stack and avoid drifting versions between test projects without a reason.
...
- Reading stale docs and assuming `AgendaPro.Tests` is still on `net9.0` or that `Directory.Build.props` defaults to `net8.0`.
